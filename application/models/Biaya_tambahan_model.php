...
class Biaya_tambahan_model extends CI_Model
{
    ...
    public function getOneBy($where = array()){
        $this->db->select("biaya_tambahan.*")->from("biaya_tambahan"); 
        $this->db->where($where);  
        $this->db->where("biaya_tambahan.is_deleted",0);  
    ...
    public function update($data,$where){
        $this->db->update("biaya_tambahan", $data, $where);
        return $this->db->affected_rows();
    }

    public function delete($where){
        $this->db->where($where);
        $this->db->delete("biaya_tambahan"); 
